...
class Fuzzy
{
	...
	private function _initialize_variable()
	{
		...
		$this->_m_vars = $this->_ci()->variabel;
...
//				$t = $alpha * ($max - $min) + $min;
		...
		foreach ($f_defuzzed as $id => $defuzz) {
			if (isset($data->{$id})) {
				$sid = $id - 1;
				$lahan = $data->{$sid};
				$lahan->defuzzed = $defuzz;
				$inc = strpos(strtolower(serialize($lahan)), $exclude[0][1]) && strpos(strtolower(serialize($lahan)), $exclude[1][1]) && strpos(strtolower(serialize($lahan)), $exclude[2][1]);
				if ($inc)
					$x_defuzzed[] = $lahan;
			}
		}
...
//		self::log($defuzzed);
		...
		foreach ($f_defuzzed as $id => $defuzz) {
			$ci =& get_instance();
			$ci->db->insert('rekomendasi',array('id_lahan'=>$id,'nilai'=>round($defuzz,3)));
		}
		$this->_defuzzed = $x_defuzzed;
	}
	...
	private function _initialize_rules()
	{
		...
		foreach ($m_rule->get_all() as $var) {
			$this->_rules[] = ['ketinggian' => $var->ketinggian, 'suhu' => $var->suhu, 'hujan' => $var->hujan, 'tanah' => $var->tanah, 'nilai' => $var->nilai];
		}
	}
